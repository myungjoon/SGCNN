...
for n in range(1):
    data_file_name_Training = 'data.txt'
    data_file_name_Test = 'test.txt'

    ## training file
    with open(data_file_name_Training, 'r') as file:
        ...
        for i in range(len(lines)):
            ...
            training_y_data.append(float(str_list[6]))

        training_materials = np.array(training_materials)
        training_y_data = np.array(training_y_data)

    permutation = np.arange(len(training_materials))
    np.random.shuffle(permutation)
    training_materials = training_materials[permutation]
    training_y_data = training_y_data[permutation]

    ##test file
    with open(data_file_name_Test, 'r') as file:
        lines = file.readlines()
        test_materials = []
        test_y_data = []
    ...
    training_bulk_num, training_atom_b, training_bond_b, training_bond_index1_b, training_bond_index2_b, training_atom_num_b, training_bond_num_b, \
    training_surface_num, training_atom_s, training_bond_s, training_bond_index1_s, training_bond_index2_s, training_atom_num_s, training_bond_num_s = data_preparation(
        training_materials)
    ...
    with tf.Session() as sess:
        ...
        training_y = np.reshape(training_y_data, [-1, 1])
        test_y = np.reshape(test_y_data, [-1, 1])
        
        training_costs = []
        test_costs = []

        feed_dict_training = {atom_num_b: training_bulk_num, atom_num_s: training_surface_num, \
                              atoms_b: training_atom_b, bonds_b: training_bond_b, atom_number_b: training_atom_num_b,
                              bond_number_b: training_bond_num_b, \
                              bonds_b_index1: training_bond_index1_b, bonds_b_index2: training_bond_index2_b, \
                              atoms_s: training_atom_s, bonds_s: training_bond_s, atom_number_s: training_atom_num_s, \
                              bond_number_s: training_bond_num_s, bonds_s_index1: training_bond_index1_s,
                              bonds_s_index2: training_bond_index2_s, \
                              y: training_y, keep_prob: dropout_prob}
        ...
        for epoch in range(epochs):
            batch_num = int(training_materials.shape[0] / batch_size)
            for i in range(batch_num):
                index = np.arange(i * batch_size, (i + 1) * batch_size)

                batch_index1_b = training_bond_index1_b[index]
                batch_index2_b = training_bond_index2_b[index]

                batch_index1_s = training_bond_index1_s[index]
                batch_index2_s = training_bond_index2_s[index]

                shift_b = batch_index1_b.flatten()[0]
                shift_s = batch_index1_s.flatten()[0]

                batch_index1_b[batch_index1_b > -1] -= shift_b
                batch_index2_b[batch_index2_b > -1] -= shift_b

                batch_index1_s[batch_index1_s > -1] -= shift_s
                batch_index2_s[batch_index2_s > -1] -= shift_s

                feed_dict_batch = {atom_num_b: training_bulk_num, atom_num_s: training_surface_num, \
                                   atoms_b: training_atom_b[index], bonds_b: training_bond_b[index], \
                                   atom_number_b: training_atom_num_b[index], bond_number_b: training_bond_num_b, \
                                   bonds_b_index1: batch_index1_b, bonds_b_index2: batch_index2_b, \
                                   atoms_s: training_atom_s[index], bonds_s: training_bond_s[index],
                                   atom_number_s: training_atom_num_s[index], \
                                   bond_number_s: training_bond_num_s, bonds_s_index1: batch_index1_s,
                                   bonds_s_index2: batch_index2_s, \
                                   y: training_y[index], keep_prob: dropout_prob}

                c, _ = sess.run([cost, optimizer], feed_dict=feed_dict_batch)

            if epoch % 50 == 0:
                test_cost = sess.run(cost, feed_dict=feed_dict_test)
                training_cost = sess.run(cost, feed_dict=feed_dict_training)
                print('Epoch:', '%d' % (epoch + 1), 'cost = ', '{:.5f}'.format(training_cost), 'test = ',
                      '{:.5f}'.format(test_cost))
                training_costs.append(training_cost)
                test_costs.append(test_cost)

        saver.save(sess, saver_path + model_name)
        ...
        print("Test MAE : %.5f RMSE : %.5f" % (mae, rmse))


print("Average Test MAE : %.5f RMSE : %.5f" % (mae_avg/K, rmse_avg/K))
